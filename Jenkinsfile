  ...
  stages {
    ...
    stage('Setup Virtual Environment & Install Deps') {
      ...
      stage('Login to AWS ECR') {
        steps {
          sh '''
         aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 390844761974.dkr.ecr.us-east-1.amazonaws.com '''
        }
      }
      ...
      stage('Tag Docker Image') {
        steps {
          ...
          390844761974. dkr.ecr.$AWS_REGION.amazonaws.com / $ECR_REPO: $ {
            BUILD_NUMBER
          }
